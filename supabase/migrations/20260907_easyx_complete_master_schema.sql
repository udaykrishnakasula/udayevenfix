...
USING (bucket_id = 'kyc-documents' AND ((auth.uid())::text = (storage.foldername(name))[2] OR is_admin()));
...
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'kyc-selfies' AND (auth.uid())::text = (storage.foldername(name))[2]);
...
ON storage.objects FOR SELECT
USING (bucket_id = 'kyc-selfies' AND ((auth.uid())::text = (storage.foldername(name))[2] OR is_admin()));

DROP POLICY IF EXISTS "Users can upload deposit proofs" ON storage.objects;
CREATE POLICY "Users can upload deposit proofs"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'deposit-proofs' AND (auth.uid())::text = (storage.foldername(name))[2]);
...
ON storage.objects FOR SELECT
USING (bucket_id = 'deposit-proofs' AND ((auth.uid())::text = (storage.foldername(name))[2] OR is_admin()));
