...
import { money, useBuyPlan, usePublicMaintenance } from "@/user/api";
...
export default function BuyPlanDialog({ plan, open, onOpenChange, walletBalance }) {
  ...
  const isInvestBlocked =
    Boolean(maintenance?.is_enabled) || maintenance?.features?.investments === false;
  ...
  const onBuy = async () => {
    if (isInvestBlocked) {
      toast.error(maintenance?.message || "Investments are currently disabled for maintenance.");
      return;
    }
  ...
  return (
    <EasyXModal
      ...
      footer={
        <EasyXButton
          onClick={onBuy}
          disabled={isInvestBlocked || insufficient || buy.isPending}
          loading={buy.isPending}
          data-testid={`buy-confirm-${plan.key}`}
          className="w-full"
        >
          {isInvestBlocked ? (
            <><AlertTriangle className="mr-2 h-4 w-4" /> Investment unavailable</>
          ) : insufficient ? (
    ...
    >
      {isInvestBlocked && (
        <div className="rounded-ex-ctrl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200 flex items-start gap-2.5 mb-3">
          <AlertTriangle className="h-4 w-4 shrink-0 text-amber-400 mt-0.5" />
          <div>
            <div className="font-semibold text-amber-300">Investments Temporarily Paused</div>
            <div className="text-amber-200/80 mt-0.5">
              {maintenance?.message || "New plan purchases are currently disabled for maintenance."}
            </div>
          </div>
        </div>
      )}
      ...
      {insufficient && !isInvestBlocked && (
        <div className="rounded-ex-ctrl border border-red-500/30 bg-red-500/10 p-3 text-sm mt-3" data-testid={`buy-insufficient-${plan.key}`}>
